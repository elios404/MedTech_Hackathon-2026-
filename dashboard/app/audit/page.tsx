...
export default function AuditPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="text-xs text-slate-500 font-mono flex items-center gap-1.5 mb-1">
            ...
            <span>/</span>
            <span className="text-slate-800 font-semibold">Smart Cart Audit</span>
          </div>
          <h2 className="text-xl font-bold text-slate-900 tracking-tight">
            Non-Destructive Bulk Density Audit & Anomaly Detection
          </h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Physical load cell & volume audit enforcing AS/NZS 3816 standards without unsealing bags.
          </p>
        </div>

        {/* Status Callout */}
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-red-50 border border-red-200 text-xs text-red-800 self-start md:self-auto shadow-sm">
          <AlertOctagon className="w-4 h-4 text-red-600 shrink-0" />
          <span className="whitespace-nowrap">Active Quarantined Bags: <strong>4 Bags Isolated</strong></span>
        </div>
      </div>

      {/* 1. Bulk Density Scatter Plot & Physics Explanation */}
          ...
          <div className="space-y-4">
            ...
            <div className="p-3 rounded-lg bg-amber-50 border border-amber-200 text-xs space-y-1.5">
              ...
              <p className="text-slate-700 text-[11px] leading-relaxed">
                Indicates yellow biohazard bag filled with lightweight sterile plastic packaging and trapped air. Direct contributor to clinical budget leakage.
              </p>
            </div>

            <div className="p-3 rounded-lg bg-red-50 border border-red-200 text-xs space-y-1.5">
          ...
          <div className="flex items-center gap-1.5 text-xs bg-slate-100 p-1 rounded-lg border border-slate-200 self-start">
            {["ALL", "ANOMALY", "LOW_DENSITY", "HIGH_DENSITY"].map((f) => (
              <button
                ...
                className={`px-2.5 py-1 rounded-md text-[11px] font-mono font-semibold transition-colors whitespace-nowrap ${
                  filterType === f
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-200 text-slate-500 font-bold uppercase tracking-wider text-[10px] bg-slate-50">
                <th className="py-3 px-4">Bag ID / RFID</th>
                <th className="py-3 px-4">Origin Theatre</th>
                <th className="py-3 px-4">Bin Stream</th>
                <th className="py-3 px-4">Gross Weight</th>
                <th className="py-3 px-4">Volume</th>
                <th className="py-3 px-4">Bulk Density</th>
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {filteredLogs.map((bag) => (
                ...
                >
                  ...
                  <td className="py-3 px-4 font-mono text-[11px] text-slate-600 whitespace-nowrap">{bag.binType}</td>
                  <td className="py-3 px-4 font-mono whitespace-nowrap">{formatWeightKg(bag.grossWeightKg)}</td>
                  <td className="py-3 px-4 font-mono whitespace-nowrap">{formatVolumeL(bag.measuredVolumeL)}</td>
      ...
      {/* 3. Detail Drawer / Modal for Selected Bag */}
      {selectedBag && (
